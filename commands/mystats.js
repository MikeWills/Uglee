...
exports.handler = function(data, userid, source) {
        ...
        client.query('SELECT @rank := rank FROM (SELECT @rownum := @rownum + 1 AS ' + 'rank, djid, POINTS FROM (SELECT djid, sum(up) as POINTS from ' + dbName + '.' + dbTablePrefix + 'Song WHERE `roomid` = ? group by djid order by sum(up) desc) as test) as rank where ' + 'djid like \'' + data.userid + '\'', [currentRoomId]);
