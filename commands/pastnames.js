...
exports.handler = function(data, userid, source) {
    ...
    client.query('SELECT username FROM ' + dbName + '.' + dbTablePrefix + 'User  WHERE `roomid` = ? AND (userid like (SELECT ' + 
        'userid FROM ' + dbName + '.' + dbTablePrefix + 'User  WHERE username LIKE ? limit 1)) ORDER BY RAND()', 
    [currentRoomId, data.text.substring(11)], function select(error, results, fields) {
        var response = '';
