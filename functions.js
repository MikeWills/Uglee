...
global.SetCacheValue = function(key, value) {
	if (useDB) {
		client.query("SELECT `value` FROM " + dbTablePrefix + "CACHE WHERE `key` = ?", [key], function select(error, results, fields) {
			if (results !== undefined) {
				if (results.length !== 0) {
					client.query("UPDATE " + dbTablePrefix + "CACHE SET `value` = ? WHERE `key` = ?", [value, key]);
				} else {
					client.query("INSERT INTO " + dbTablePrefix + "CACHE (`key`, `value`, `DateStamp`) VALUES (?, ?, CURRENT_TIMESTAMP)", [key, value]);
				}
			} else {
				client.query("INSERT INTO " + dbTablePrefix + "CACHE (`key`, `value`, `DateStamp`) VALUES (?, ?, CURRENT_TIMESTAMP)", [key, value]);
			}
...
global.GetCacheValue = function(key, timeout, callback) {
	if (useDB) {
		client.query("SELECT `value`, `DateStamp` FROM " + dbTablePrefix + "CACHE WHERE `key` = ?", [key], function select(error, results, fields) {
			Log("Results: " + results);
			if (results !== undefined) {
				if (results.length !== 0) {
					var now = new Date();
...
global.RemoveCacheValue = function(key) {
	if (useDB) {
		client.query("DELETE FROM " + dbTablePrefix + "CACHE WHERE `key` = ?", [key]);
	}
};
...
global.SetUpDatabase = function() {
	...
	client.query("CREATE TABLE IF NOT EXISTS `" + dbTablePrefix + "Cache` (`key` varchar(50) NOT NULL, `value` varchar(4096) NOT NULL, `DateStamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)", function(error) {
		//Handle an error if it's not a table already exists error
		if (error && error.number != 1050) {
			throw (error);
		}
